...
if (isset($_POST['next'])) {
    header('Refresh: 1;URL=modifgentile.php');
}

if (isset($_POST['chiant'])) {
    $data = [
...
if ($i == 0) {
    ...
    try {
        ...
        $sql = 'SELECT id_ville,nom_ville, gentile_h_ville, gentile_f_ville FROM ville WHERE gentilemodif is null and gentile_h_ville is not null LIMIT 1' ;
        $sth = $pdo->prepare($sql);
        $sth->execute();
    ...
    <form method = "POST" action = "modifgentile.php" enctype="multipart/form-data" >
        <fieldset >
            <div class="form-group">
                <label class="col-md-4 control-label"></label>
                <div class="col-md-4">
                    <button style="padding-left: 50%; padding-right: 50%;" type="submit" name="chiant" class="btn btn-danger" >💩<span class="glyphicon glyphicon-send"></span></button>
                </div>
                </div>
        </fieldset>
    </form>
